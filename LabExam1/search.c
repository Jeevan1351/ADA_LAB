...
int mode(int k, int arr[10], int n)
{
    int i = BinarySearch(arr, 0, (int)(n-1)/2, n-1, k);
    int j = i-1;
    int m = i+1;
    int c = 1;
    while(arr[j] == k)
...
int main()
{
	...
	while(ch!=2)
	{
		printf("\n\n\n\t1]Linear Search AndBinary Search\n\t2]Exit\nEnter Choice:\t");
		scanf("%d", &ch);
		if(ch==2)
			break;
    ...
    scanf("%d", &k);
    printf("The mode of the given key is:\t%d", mode(k, arr, n));
